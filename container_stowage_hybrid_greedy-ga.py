...
def decode_greedy_optimized(ship, chromosome, sorted_keys):
    for s in ship.slots:
        s.container = None
    ship.filled_indices = []
    ship.invalidate_cache()
    conts_40 = [c for c in chromosome if c.size == 40]
    conts_20 = [c for c in chromosome if c.size == 20]
    n20, n40 = 0, 0
...
def _stab_penalty_teu(value, target, a_lin, a_quad, tier2_teu, tier3_teu,
    ...
    """
    Penalti stabilitas non-linear bertingkat -- dalam satuan TEU ekuivalen.

    Semua bobot dinyatakan sebagai "kehilangan N TEU" sehingga fitness
    tetap dalam skala TEU dan SELALU POSITIF selama ada kontainer.

    Formula:
        p = a_lin * d  +  a_quad * d^2  [+ tier2_teu jika d>thresh2]
                                        [+ tier3_teu jika d>thresh3]
    di mana d = max(0, |value - target| - tol)

    Contoh interpretasi:
        a_lin=80   -> setiap 1 m deviasi = kehilangan 80 TEU
        tier2_teu=200 -> jika deviasi >thresh2 m, langsung -200 TEU lagi
    """
...
_BASE_OFFSET = 2000  # TEU ekuivalen -- jangkar agar fitness selalu > 0

def calculate_fitness(ship, n20, n40):
    v, t, l, _ = ship.calculate_stability()
    total_teu = n20 + (n40 * 2)

    # -- Reward utama: 1 poin per TEU -------------------------------------
    reward = float(total_teu)

    # -- VCG: target 9.0 m ------------------------------------------------
    # Setiap 1 m deviasi = -80 TEU; kuadratik +60/m^2
    # Threshold:  >0.5 m -> -200 TEU;  >1.0 m -> -400 TEU tambahan
    v_penalty = _stab_penalty_teu(v, 9.0,
                                   a_lin    =  80.0,
                                   a_quad   =  60.0,
                                   tier2_teu= 200.0,
                                   tier3_teu= 400.0,
    ...
    # -- LCG: target 2.0 m ------------------------------------------------
    # Setiap 1 m deviasi = -40 TEU; kuadratik +20/m^2
    # Threshold:  >0.3 m -> -60 TEU;  >1.0 m -> -150 TEU tambahan
    l_penalty = _stab_penalty_teu(l, 2.0,
                                   a_lin    =  40.0,
                                   a_quad   =  20.0,
                                   tier2_teu=  60.0,
                                   tier3_teu= 150.0,
    ...
    # -- TCG: target 0.0 m ------------------------------------------------
    # Setiap 1 m deviasi = -20 TEU; kuadratik +15/m^2
    # Threshold:  >0.3 m -> -30 TEU;  >1.0 m -> -80 TEU tambahan
    t_penalty = _stab_penalty_teu(t, 0.0,
                                   a_lin    =  20.0,
                                   a_quad   =  15.0,
                                   tier2_teu=  30.0,
                                   tier3_teu=  80.0,
...
# ==========================================
# 4. SINGLE GA RUN (DENGAN PROGRESS REPORTING)
# ==========================================
...
def run_single_ga(args):
    ...
    target_ap = 44.8
    sorted_keys = sorted(ship_tmplt.slot_map.keys(), 
                         key=lambda k: (k[2], abs(ship_tmplt.slots[ship_tmplt.slot_map[k]].lcg - target_ap), abs(k[1]-3)))
    
    # EARLY STOPPING: hentikan jika tidak ada improvement selama N generasi
    no_improve_streak = 0
    EARLY_STOP_LIMIT = 5  # Stop jika n gen berturut-turut tidak ada kemajuan
    prev_best_fit = float('-inf')

    for gen in range(generations):
        scored = []
        for chrom in population:
            ship, n20, n40 = decode_greedy_optimized(ship_tmplt, chrom, sorted_keys)
        ...
        # Evolution -- ELITISME LEBIH KUAT (top 8) + MUTATION RATE ADAPTIF
        elite_count = 8
        next_gen = [scored[i]['chrom'] for i in range(min(elite_count, len(scored)))]
        mut_rate = 0.15 + 0.15 * (no_improve_streak / EARLY_STOP_LIMIT)  # Tingkatkan mutasi saat stagnan
    ...
    best_ship, f_n20, f_n40 = decode_greedy_optimized(ship_tmplt, best_overall['chrom'], sorted_keys)
...
# ==========================================
# 5. DISPLAY UTILITIES (INFORMATIF)
# ==========================================
...
def print_header():
    print(color("+==========================================================================+", "1;36"))
    print(color("|          SHIP STOWAGE PLAN OPTIMIZER  --  GA ENGINE v3.0             |", "1;36"))
    print(color("+==========================================================================+", "1;36"))
    print()

def print_config_summary(total_conts, c20, c40, n_runs, generations, pop_size, n_cpu):
    print(color("+-------------------------------------------------------------------------+", "90"))
    print(color("|  KONFIGURASI OPTIMASI                                                   |", "90"))
    print(color("+----------------------+--------------------------------------------------+", "90"))
    ...
    print(color(f"|  [PEN]   Penalti VCG     |", "90") + f"  80/m + 60/m^2 | >0.5m:-200 | >1.0m:-400 [TEU]     " + color("|", "90"))
    print(color(f"|  [PEN]   Penalti LCG     |", "90") + f"  40/m + 20/m^2 | >0.3m: -60 | >1.0m:-150 [TEU]     " + color("|", "90"))
    print(color(f"|  [PEN]   Penalti TCG     |", "90") + f"  20/m + 15/m^2 | >0.3m: -30 | >1.0m: -80 [TEU]     " + color("|", "90"))
    print(color(f"|  [FIT]   Fitness Basis   |", "90") + f"  offset 2000 + TEU - penalti  (selalu > 0)         " + color("|", "90"))
    print(color(f"|  [STOP]  Early Stopping  |", "90") + f"                                                    " + color("|", "90"))
    print(color("+----------------------+--------------------------------------------------+", "90"))
    print()

def print_live_dashboard(run_states, total_runs, total_gen, global_best, elapsed):
    print(color("+-------------------------------------------------------------------------+", "90"))
    print(color("|  [LIVE] PROGRESS MONITOR                                                |", "90"))
    print(color("+------+----------------------------------+--------+----------------------+", "90"))
    ...
    for run_id in sorted(run_states.keys()):
        ...
        gen_cur = st.get('gen', 0)
        gen_tot = st.get('total_gen', total_gen)
        teu = st.get('teu', 0)
        fit = st.get('best_fit', 0)
        stopped = st.get('stopped', False)
        stop_gen = st.get('stop_gen', gen_cur)
...
def print_comparison_table(all_results):
    print()
    print(color("+==========================================================================================+", "1;34"))
    print(color("|                             RINGKASAN SEMUA RUN                                           |", "1;34"))
    print(color("+=======+=======+===========+=========+============+=========+===========================+", "34"))
...
def display_results_final(ship, n20, n40, exec_time):
    ...
    def delta_indicator(value, target, threshold_ok=0.3, threshold_warn=1.0):
        ...
        if d < threshold_ok:    return color(f"{arrow} D{d:.3f}", "32")  # Hijau
        elif d < threshold_warn: return color(f"{arrow} D{d:.3f}", "33")  # Kuning
        else:                    return color(f"{arrow} D{d:.3f}", "31")  # Merah
    
    print()
    print(color("+==========================================================================+", "1;32"))
    print(color("|              [BEST] HASIL TERBAIK -- STOWAGE PLAN OPTIMAL               |", "1;32"))
    print(color("+============================+=============================================+", "32"))
...
def display_bay_layout(ship):
    """Tampilkan layout penempatan kontainer per Bay (dari kode asli, dengan warna)"""
    ...
    for b in bays_available:
        ...
        # Urutan row: Port terluar -> Starboard terluar (sama dengan Program B & C)
        ...
        for tier in tiers_in_bay:
            ...
            for row in rows_in_bay:
                ...
                if key in ship.slot_map:
                    ...
                    if c:
                        if c.size == 40:
                            label = color(f"{c.weight:4.1f}t", "1;35")  # Magenta untuk 40ft
                        else:
                            label = color(f"{c.weight:4.1f}t", "1;32")  # Hijau untuk 20ft
                    else:
                        label = color("  .  ", "90")  # Abu-abu untuk kosong
                else:
                    label = color(" XXX ", "31")  # Merah untuk slot tidak tersedia
...
def export_to_excel(ship, filename):
    ...
    for s in sorted(ship.slots, key=lambda x: (x.tier, x.bay, x.row)):
        if s.container and (s.bay, s.row, s.tier) not in processed_slots:
            ...
            rows.append({
                'Slot ID': f"B{s.bay:02d}R{s.row:02d}T{s.tier:02d}", 
            ...
            if c.size == 40 and s.pasangan_bay != 0:
                ...
                if key_pasangan in ship.slot_map:
                    ...
                    rows.append({
                        'Slot ID': f"B{s2.bay:02d}R{s2.row:02d}T{s2.tier:02d}", 
                        'Container ID': c.id, 'Weight (Ton)': weight_per_bay,
    ...
    v, t, l, tot = ship.calculate_stability()
    summary_rows = [
        {'Slot ID': '', 'Container ID': '', 'Weight (Ton)': '', 'Size': '', 'VCG': '', 'TCG': '', 'LCG': ''},
        {'Slot ID': 'SUMMARY', 'Container ID': '', 'Weight (Ton)': '', 'Size': '', 'VCG': '', 'TCG': '', 'LCG': ''},
...
# ==========================================
# 5b. UNIFIED COMPARISON SUMMARY
# ==========================================

def print_unified_summary(label, n20, n40, vcg, tcg, lcg):
    """Cetak ringkasan dalam format seragam untuk perbandingan antar program."""
    total_teu = n20 + n40 * 2
    reward    = float(total_teu)
...
if __name__ == "__main__":
    ...
    # 3. Konfigurasi GA -- diset ketat untuk target ~10 detik
    JUMLAH_RUN  = 20
    POP_SIZE    = 50
    GENERATIONS = 50
    BASE_SEED   = 42
    ...
    # 5. Manager untuk progress real-time
    manager = Manager()
    progress_queue = manager.Queue()
    
    args_list = [(i+1, ship_data, conts_data, POP_SIZE, GENERATIONS, BASE_SEED, progress_queue) for i in range(JUMLAH_RUN)]

    # 6. Jalankan paralel + tampilkan progress
    ...
    with Pool(processes=N_CPU) as pool:
        ...
        last_refresh = 0
        REFRESH_RATE = 0.5  # Refresh tampilan setiap 0.5 detik
    ...
    # Tampilkan final dashboard dengan semua run selesai
    # Update run_states dari hasil final
    for res in all_results:
        rid = res['run']
        if rid not in run_states:
            run_states[rid] = {}
    ...
    output_file = f"E:/Documents/ITS/Math/Semester 5/Kerja Praktik/work/verif8_{filename_no_ext}.xlsx"
    ...
    print()
    # -- Unified summary untuk perbandingan ------------------------------
